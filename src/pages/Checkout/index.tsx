...
const Checkout = () => {
  ...
  const form = useFormik({
    // Passamos os campos do formulário e seus valores
    initialValues: {
      fullName: '',
      email: '',
      cpf: '',
    ...
    // validationSchema faz a validação dos campos
    // colocamos todos os campos dentro do Yup.object para ele validar
    ...
    // Função que é ativada quando o formulário for Submit
    onSubmit: (values) => {
      // Aplicando os valores na requisição
      purchase({
        billing: {
          document: values.cpf,
          email: values.email,
  ...
  // O formik fornece uma variedade de informações sobre o formulário
  // console.log(form)

  const checkInputHasError = (fieldName: string) => {
    // o in verifica se o campo está presente no objeto touched
  ...
  if (items.length === 0 && isSuccess === false) {
    // Componente que só serve para fazer o redirecionamento
    return <Navigate to="/" />
  }
              ...
              <S.Row>
                <S.InputGroup>
                  ...
                  <input
                    type="text"
                    id="fullName"
                    // Definindo o formik para controlar o valor e a mudança do campo
                    name="fullName"
